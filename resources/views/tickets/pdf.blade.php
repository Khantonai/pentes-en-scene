...
<body>
    ...
    <h1>Billet pour {{ $ticket->first_name }} {{ $ticket->last_name }}</h1>
    @if($ticket->price > 0)
        ...
        <p style="position:absolute; right:450px;  bottom:677px; ">   {{ \Carbon\Carbon::parse($ticket->date_start)->format('d/m/y') }} </p>
        <p style="position:absolute; right:455px;  bottom:655px; ">   {{ $ticket->last_name }} </p>
        <p style="position:absolute; right:445px;  bottom:630px; ">   {{ $ticket->first_name }} </p>
    @else
